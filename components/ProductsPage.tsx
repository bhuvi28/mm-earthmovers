...
export default function ProductsPage({ initialCategory = 'loader', products }: ProductsPageProps) {
  ...
  const handleProductEnquire = (productName: string, oemRef: string) => {
    ...
    router.push(`/#contact`)
    // We might need a way to pass the message, but for now just linking to contact is fine.
    // Or we could use a query param ?enquire=... and handle it in Home.
  }
              ...
              filteredAndSortedProducts.map((product, index) => (
                ...
                >
                  ...
                  <div className="p-5 flex-grow flex flex-col">
                    ...
                    <button
                      onClick={() => handleProductEnquire(product.title, product.oemRef || '')}
