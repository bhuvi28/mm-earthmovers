import type { Metadata } from 'next'
import { Inter } from 'next/font/google'
import Script from 'next/script'
import './globals.css'

const inter = Inter({ subsets: ['latin'] })

export const metadata: Metadata = {
  metadataBase: new URL('https://www.mmearthmovers.com'),
  title: {
    default: 'MM Earthmovers - Heavy Equipment Spare Parts | Loader, Excavator & MotorGrader Parts',
    template: '%s | MM Earthmovers',
  },
  description:
    'Premium supplier of heavy earthmoving machinery spare parts in Kolkata, India. Quality replacement parts for loaders, excavators, motor graders - HM, BEML, L&T, XCMG, Komatsu, and more brands.',
  keywords: [
    // Brand keywords (GSC performing)
    'mm earthmoving spares',
    'mm spares',
    'mm earthmovers',
    // Core product keywords
    'earthmoving equipment spare parts',
    'earthmoving spare parts',
    'earthmoving spares',
    'earth moving spare parts',
    'earthmoving machinery spare parts',
    'earthmoving machinery parts',
    'heavy earth moving equipment spare parts',
    // Earthmovers keywords
    'earthmovers',
    'earthmovers parts',
    'earthmovers spare parts',
    'earthmovers equipment',
    'earth movers spare parts',
    'earth movers machinery',
    'heavy earthmovers',
    // Loader keywords
    'loader spare parts',
    'hm loader 2021',
    'HM 2021 parts',
    'HM 2021 B parts',
    'HM 2021 D parts',
    'HM 2021 E parts',
    'TATA TWL 3036 parts',

    'L&T 9020 parts',
    'l&t loader 9020 parts',
    'JCB 430 parts',
    'JCB 430ZX parts',
    'JCB 3DX parts',
    // Grader keywords
    'motor grader spare parts',
    'motor grader parts',
    'motor grader components',
    'grader spare parts',
    'grader parts',
    'grader spares',
    'road grader parts',
    // Excavator keywords
    'excavator parts',
    'excavator spare parts',
    // Brand keywords
    'HM parts',
    'BEML BG 605 parts',
    'BEML BG 511 parts',
    'XCMG parts',
    'LIUGONG parts',
    'SDLG parts',
    'Komatsu parts',
    'jcb regulator',
    // Location & business
    'heavy equipment parts',
    'spare parts',
    'construction equipment parts',
    'Kolkata',
    'India',
    'global shipping',
  ],
  authors: [{ name: 'MM Earthmovers' }],
  creator: 'MM Earthmovers',
  publisher: 'MM Earthmovers',
  formatDetection: {
    email: false,
    address: false,
    telephone: false,
  },
  openGraph: {
    type: 'website',
    locale: 'en_IN',
    url: 'https://www.mmearthmovers.com',
    siteName: 'MM Earthmovers',
    title: 'MM Earthmovers - Heavy Equipment Spare Parts Supplier',
    description:
      'Premium supplier of heavy earthmoving machinery spare parts. Quality replacement parts for loaders, excavators, motor graders in Kolkata, India.',
    images: [
      {
        url: '/og-image.jpg',
        width: 1200,
        height: 630,
        alt: 'MM Earthmovers - Heavy Earthmoving Machinery Spare Parts',
      },
    ],
  },
  twitter: {
    card: 'summary',
    title: 'MM Earthmovers - Heavy Equipment Spare Parts',
    description: 'Premium supplier of loader, excavator, and motor grader spare parts in India.',
    images: ['/og-image.jpg'],
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      'max-video-preview': -1,
      'max-image-preview': 'large',
      'max-snippet': -1,
    },
  },
  icons: {
    icon: '/favicon.ico',
    apple: '/logo.png',
  },
  verification: {
    // Add Google Search Console verification code here when available
    // google: 'your-google-verification-code',
  },
}

import WhatsAppButton from '@/components/WhatsAppButton'

import { Analytics } from "@vercel/analytics/next"

export default function RootLayout({
  children,
}: {
  children: React.ReactNode
}) {
  return (
    <html lang="en" className="scroll-smooth light">
      <head>
        {/* Google tag (gtag.js) */}
        <Script
          src="https://www.googletagmanager.com/gtag/js?id=G-RHFEC523K9"
          strategy="afterInteractive"
        />
        <Script id="google-analytics" strategy="afterInteractive">
          {`
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', 'G-RHFEC523K9');
          `}
        </Script>
      </head>
      <body className={`${inter.className} antialiased bg-white`}>
        {children}
        <WhatsAppButton />
        <Analytics />
      </body>
    </html>
  )
}
